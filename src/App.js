...
//   {id: uuidv4(), charge: "car", amount:1500 },
//   {id: uuidv4(), charge: "AEDC Bill", amount:1200 }
// ]; 
  ...
  if(charge !== '' && amount > 0){
    ...
    if(edit){
      let tempExpenses = expenses.map(item =>{
        return item.id === id ? {...item, item, charge, amount} :item
      });
      setExpenses(tempExpenses);
      setEdit(false);
    ...
    handleAlert({type:'danger', text:'charge can not be empty and amount has to be bigger than zero'});
  }
}
